...
def gyro_sensor(name=str(),queue=None):
    try:
        ...
        keys=['roll', 'pitch', 'yaw', 'accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z','time','time_end']
        ...
        while(True):
            try:
                ...
                g_data[keys[9]].append(now)
                
                
                writecsv2.writedatacsv(name,g_data,1)
                    
                
                message=g_data#str(str(e[0])+","+str(e[1])+","+str(e[2])+","+str(nr)+","+str(now)) #the whole contain must converted to a string!
                
                
                #print(message)
                nr+=1
                time.sleep(abs(0.05-(time.time()-start)))
        ...
        fname=os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        failkey=['exc_type', 'fname','exc_tb.tb_lineno']
        fail[0].append(exc_type)
        fail[1].append(fname)
        fail[2].append(exc_tb.tb_lineno)
        writecsv2.writedatacsv(name,fail,1)
        print(fail)
        
        
    finally:
        g_data[keys[10]].append(time.time())
        writecsv2.writedatacsv(name,g_data,1)
        print('end Gyro',g_data)
